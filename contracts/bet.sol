...
contract CSBetting is ERC721, Functional, Owned
{
	...
	event LogToken(string _event, address user, uint32 idBetting, uint32 idToken, uint8 betType, uint32 betVal, uint256 amount);
	...
	function getBettingByID(uint32 _id) public view returns (
		...
		betsCount = getCountTokensByBetting(_id);
		betsSumIn = gm.betsSumIn;  
		if (betsSumIn==0) betsSumIn = getSumInByBetting(_id);
	...
	function getTokenByID(uint256 _id) public view returns ( 
		...
		idBetting   = uint32((packed >> (8*8)) & 0xFFFFFFFF);
		bet = uint32((packed >> (4*8)) & 0xFFFFFFFF);
		...
		if (betsSumIn==0) betsSumIn = getSumInByBetting(idBetting);
	...
	function buyToken(uint32 idBetting, uint32 betVal, uint8 betType) payable public //  //address captainAddress
	{
	    //temp 
		...
		if ( betType == 0 || betType == 1  ) { 
		    betsAll[idBetting][betVal].sum += userStake;
		    betsAll[idBetting][betVal].count += 1; 
		}
		                     

		uint128 packed;
		packed = ( uint128(idBetting) << 8*8 ) + ( uint128(betVal) << 4*8 ) + uint128(block.timestamp);
		...
		LogToken( "Buy", msg.sender, idBetting, uint32(newTokenId), betType, betVal, userStake);
	}
	
	function setRate(uint256 _ex1, uint256 _ex2, uint256 _ex3, uint256 _ex4, uint256 _ex5, uint256 _exD, uint256 _exAvg){
	    curRate = _exAvg;
	...
	function redeemToken(uint256 _tokenId) public 
	{
		...
		uint32 idBetting = uint32((packed >> (8*8)) & 0xFFFFFFFF);
		uint32 betVal = uint32((packed >> (4*8)) & 0xFFFFFFFF);
		...
		//if ( curGame.status == Status.PAYING ) sumPayment = curGame.betsSumIn * tkn.price / betsAll[idBetting][curGame.winCombination].sum;

		payout = true;
		// change tkn.option status tkn.payout = 1 
	...
	function setRatebyOracle(uint256 _rate) onlyOracle public
	{
	    curRate = _rate;
	}
	...
	function checkWinNobody(uint32 idBetting) internal
	{
		Game storage curGame = game[idBetting];
		
		curGame.status = Status.PAYING;
		curGame.betsSumIn = getSumInByBetting(idBetting);
		// mark win betsSumIn
		//for 
		// nobody win = send all to feeBetting
		/*if ( betsAll[idBetting][curGame.winCombination].count == 0 )
		{
			if (curGame.betsSumIn+curGame.feeValue!=0) feeBetting = feeBetting + curGame.betsSumIn + curGame.feeValue;
			LogEvent( "NOBODYWIN", curGame.nameBetting, curGame.betsSumIn+curGame.feeValue );
		}
		else 
			takeFee(idBetting);
		*/
